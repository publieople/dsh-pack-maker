...
import { exportPack, importPack, importPackFile, listProfiles, summarizePack } from './core.js';
...
export function apply(ctx) {
  ...
  pack
    ...
    .option('--no-check', 'skip the plugin compatibility preflight')
  ...
  pack
    ...
    .argument('[query]', 'optional search filter')
    .action((query) => {
      listMarketItems({ workspace: process.cwd() })
        .then(({ items }) => {
        ...
        .catch((error) => {
          console.error(`pack market: ${error instanceof Error ? error.message : error}`);
          done(1);
        });
    });

  pack
    .command('market-import <pathOrUrl>')
    .description('import a .dshpack from the market (a local path or a published URL) as a new profile')
    .option('-n, --name <profileName>', 'target profile name (default: the name stored in the pack)')
    .option('--overwrite', 'replace an existing profile (old directory moved to a .bak backup)')
    .option('--no-install', 'do not run pnpm install after restoring files')
    .action((pathOrUrl, options) => {
      const item = /^https?:\/\//.test(pathOrUrl)
        ? { source: 'remote', url: pathOrUrl }
        : { source: 'local', path: pathOrUrl };
      resolvePackBuffer(item)
        .then((buffer) => importPack({
          buffer,
          ...(options.name !== undefined ? { profileName: options.name } : {}),
          overwrite: options.overwrite,
          autoInstall: options.install,
        ...
        .catch((error) => {
          console.error(`pack market-import: ${error instanceof Error ? error.message : error}`);
          done(1);
        });
    });
