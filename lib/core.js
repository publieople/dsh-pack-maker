...
export async function exportPack(options) {
  ...
  delete buildOptions.check;

  let compatibility;
  if (check) {
    const { checkProfileCompat } = await import('./check.js');
    const dir = profileDir(options.profile);
    compatibility = await checkProfileCompat(dir);
    if (!compatibility.ok) {
      const error = new Error(
        'dsh-pack-maker: export blocked by plugin compatibility check:\n' +
        compatibility.errors.map((line) => `  - ${line}`).join('\n'),
      );
      error.code = 'PACK_COMPAT';
      error.report = compatibility;
      throw error;
    }
  }

  const outputPath = buildOptions.outputPath ?? resolve(
    buildOptions.workspace ?? process.cwd(),
    buildOptions.outputDir ?? DEFAULT_OUTPUT_DIR,
    `${buildOptions.profile}${PACK_EXTENSION}`,
  );
  const buffer = await buildPack(buildOptions);
